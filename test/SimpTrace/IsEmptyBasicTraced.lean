...
theorem isEmpty_Prop {p : Prop} : IsEmpty p ↔ ¬p := by
  simp_trace only [← not_nonempty_iff, nonempty_prop] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_01.json"

@[simp]
theorem isEmpty_pi {π : α → Sort*} : IsEmpty (∀ a, π a) ↔ ∃ a, IsEmpty (π a) := by
  simp_trace only [← not_nonempty_iff, Classical.nonempty_pi, not_forall] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_02.json"
...
theorem isEmpty_sigma {α} {E : α → Type*} : IsEmpty (Sigma E) ↔ ∀ a, IsEmpty (E a) := by
  simp_trace only [← not_nonempty_iff, nonempty_sigma, not_exists] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_03.json"

@[simp]
theorem isEmpty_psigma {α} {E : α → Sort*} : IsEmpty (PSigma E) ↔ ∀ a, IsEmpty (E a) := by
  simp_trace only [← not_nonempty_iff, nonempty_psigma, not_exists] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_04.json"

theorem isEmpty_subtype (p : α → Prop) : IsEmpty (Subtype p) ↔ ∀ x, ¬p x := by
  simp_trace only [← not_nonempty_iff, nonempty_subtype, not_exists] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_05.json"

@[simp]
theorem isEmpty_prod {α β : Type*} : IsEmpty (α × β) ↔ IsEmpty α ∨ IsEmpty β := by
  simp_trace only [← not_nonempty_iff, nonempty_prod, not_and_or] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_06.json"

@[simp]
theorem isEmpty_pprod : IsEmpty (PProd α β) ↔ IsEmpty α ∨ IsEmpty β := by
  simp_trace only [← not_nonempty_iff, nonempty_pprod, not_and_or] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_07.json"

@[simp]
theorem isEmpty_sum {α β} : IsEmpty (α ⊕ β) ↔ IsEmpty α ∧ IsEmpty β := by
  simp_trace only [← not_nonempty_iff, nonempty_sum, not_or] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_08.json"

@[simp]
theorem isEmpty_psum {α β} : IsEmpty (α ⊕' β) ↔ IsEmpty α ∧ IsEmpty β := by
  simp_trace only [← not_nonempty_iff, nonempty_psum, not_or] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_09.json"

@[simp]
theorem isEmpty_ulift {α} : IsEmpty (ULift α) ↔ IsEmpty α := by
  simp_trace only [← not_nonempty_iff, nonempty_ulift] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_10.json"

@[simp]
theorem isEmpty_plift {α} : IsEmpty (PLift α) ↔ IsEmpty α := by
  simp_trace only [← not_nonempty_iff, nonempty_plift] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_11.json"
...
theorem leftTotal_empty [IsEmpty α] : LeftTotal R := by
  simp_trace only [LeftTotal, IsEmpty.forall_iff] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_12.json"

theorem leftTotal_iff_isEmpty_left [IsEmpty β] : LeftTotal R ↔ IsEmpty α := by
  simp_trace only [LeftTotal, IsEmpty.exists_iff, isEmpty_iff] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_13.json"

@[simp]
theorem rightTotal_empty [IsEmpty β] : RightTotal R := by
  simp_trace only [RightTotal, IsEmpty.forall_iff] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_14.json"

theorem rightTotal_iff_isEmpty_right [IsEmpty α] : RightTotal R ↔ IsEmpty β := by
  simp_trace only [RightTotal, IsEmpty.exists_iff, isEmpty_iff] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_15.json"
...
theorem biTotal_iff_isEmpty_right [IsEmpty α] : BiTotal R ↔ IsEmpty β := by
  simp_trace only [BiTotal, leftTotal_empty, rightTotal_iff_isEmpty_right, true_and] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_16.json"

theorem biTotal_iff_isEmpty_left [IsEmpty β] : BiTotal R ↔ IsEmpty α := by
  simp_trace only [BiTotal, leftTotal_iff_isEmpty_left, rightTotal_empty, and_true] =>trace "test/SimpTrace/meas_out/IsEmptyBasicTraced_17.json"
