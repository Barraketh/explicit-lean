...
theorem isEmpty_Prop {p : Prop} : IsEmpty p ↔ ¬p := by
  simp_trace (out := "test/SimpTrace/isempty_out/call01.json") only [← not_nonempty_iff, nonempty_prop]

@[simp]
theorem isEmpty_pi {π : α → Sort*} : IsEmpty (∀ a, π a) ↔ ∃ a, IsEmpty (π a) := by
  simp_trace (out := "test/SimpTrace/isempty_out/call02.json") only [← not_nonempty_iff, Classical.nonempty_pi, not_forall]
...
theorem isEmpty_sigma {α} {E : α → Type*} : IsEmpty (Sigma E) ↔ ∀ a, IsEmpty (E a) := by
  simp_trace (out := "test/SimpTrace/isempty_out/call03.json") only [← not_nonempty_iff, nonempty_sigma, not_exists]

@[simp]
theorem isEmpty_psigma {α} {E : α → Sort*} : IsEmpty (PSigma E) ↔ ∀ a, IsEmpty (E a) := by
  simp_trace (out := "test/SimpTrace/isempty_out/call04.json") only [← not_nonempty_iff, nonempty_psigma, not_exists]

theorem isEmpty_subtype (p : α → Prop) : IsEmpty (Subtype p) ↔ ∀ x, ¬p x := by
  simp_trace (out := "test/SimpTrace/isempty_out/call05.json") only [← not_nonempty_iff, nonempty_subtype, not_exists]

@[simp]
theorem isEmpty_prod {α β : Type*} : IsEmpty (α × β) ↔ IsEmpty α ∨ IsEmpty β := by
  simp_trace (out := "test/SimpTrace/isempty_out/call06.json") only [← not_nonempty_iff, nonempty_prod, not_and_or]

@[simp]
theorem isEmpty_pprod : IsEmpty (PProd α β) ↔ IsEmpty α ∨ IsEmpty β := by
  simp_trace (out := "test/SimpTrace/isempty_out/call07.json") only [← not_nonempty_iff, nonempty_pprod, not_and_or]

@[simp]
theorem isEmpty_sum {α β} : IsEmpty (α ⊕ β) ↔ IsEmpty α ∧ IsEmpty β := by
  simp_trace (out := "test/SimpTrace/isempty_out/call08.json") only [← not_nonempty_iff, nonempty_sum, not_or]

@[simp]
theorem isEmpty_psum {α β} : IsEmpty (α ⊕' β) ↔ IsEmpty α ∧ IsEmpty β := by
  simp_trace (out := "test/SimpTrace/isempty_out/call09.json") only [← not_nonempty_iff, nonempty_psum, not_or]

@[simp]
theorem isEmpty_ulift {α} : IsEmpty (ULift α) ↔ IsEmpty α := by
  simp_trace (out := "test/SimpTrace/isempty_out/call10.json") only [← not_nonempty_iff, nonempty_ulift]

@[simp]
theorem isEmpty_plift {α} : IsEmpty (PLift α) ↔ IsEmpty α := by
  simp_trace (out := "test/SimpTrace/isempty_out/call11.json") only [← not_nonempty_iff, nonempty_plift]
...
theorem leftTotal_empty [IsEmpty α] : LeftTotal R := by
  simp_trace (out := "test/SimpTrace/isempty_out/call12.json") only [LeftTotal, IsEmpty.forall_iff]

theorem leftTotal_iff_isEmpty_left [IsEmpty β] : LeftTotal R ↔ IsEmpty α := by
  simp_trace (out := "test/SimpTrace/isempty_out/call13.json") only [LeftTotal, IsEmpty.exists_iff, isEmpty_iff]

@[simp]
theorem rightTotal_empty [IsEmpty β] : RightTotal R := by
  simp_trace (out := "test/SimpTrace/isempty_out/call14.json") only [RightTotal, IsEmpty.forall_iff]

theorem rightTotal_iff_isEmpty_right [IsEmpty α] : RightTotal R ↔ IsEmpty β := by
  simp_trace (out := "test/SimpTrace/isempty_out/call15.json") only [RightTotal, IsEmpty.exists_iff, isEmpty_iff]
...
theorem biTotal_iff_isEmpty_right [IsEmpty α] : BiTotal R ↔ IsEmpty β := by
  simp_trace (out := "test/SimpTrace/isempty_out/call16.json") only [BiTotal, leftTotal_empty, rightTotal_iff_isEmpty_right, true_and]

theorem biTotal_iff_isEmpty_left [IsEmpty β] : BiTotal R ↔ IsEmpty α := by
  simp_trace (out := "test/SimpTrace/isempty_out/call17.json") only [BiTotal, leftTotal_iff_isEmpty_left, rightTotal_empty, and_true]
