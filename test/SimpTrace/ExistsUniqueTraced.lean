...
theorem existsUnique_const {b : Prop} (α : Sort*) [i : Nonempty α] [Subsingleton α] :
    (∃! _ : α, b) ↔ b := by simp_trace =>trace "test/SimpTrace/meas_out/ExistsUniqueTraced_01.json"

@[simp] theorem existsUnique_eq {a' : α} : ∃! a, a = a' := by
  simp_trace only [eq_comm, ExistsUnique, and_self, forall_eq', exists_eq'] =>trace "test/SimpTrace/meas_out/ExistsUniqueTraced_02.json"

/-- The difference with `existsUnique_eq` is that the equality is reversed. -/
@[simp] theorem existsUnique_eq' {a' : α} : ∃! a, a' = a := by
  simp_trace only [ExistsUnique, and_self, forall_eq', exists_eq'] =>trace "test/SimpTrace/meas_out/ExistsUniqueTraced_03.json"

theorem existsUnique_prop {p q : Prop} : (∃! _ : p, q) ↔ p ∧ q := by simp_trace =>trace "test/SimpTrace/meas_out/ExistsUniqueTraced_04.json"
...
theorem ExistsUnique.elim₂ {p : α → Sort*} [∀ x, Subsingleton (p x)]
  ...
  simp_trace only [existsUnique_iff_exists] at h₂ =>trace "test/SimpTrace/meas_out/ExistsUniqueTraced_05.json"
  apply h₂.elim
...
theorem ExistsUnique.intro₂ {p : α → Sort*} [∀ x, Subsingleton (p x)]
  ...
  simp_trace only [existsUnique_iff_exists] =>trace "test/SimpTrace/meas_out/ExistsUniqueTraced_06.json"
...
theorem ExistsUnique.unique₂ {p : α → Sort*} [∀ x, Subsingleton (p x)]
  ...
  simp_trace only [existsUnique_iff_exists] at h =>trace "test/SimpTrace/meas_out/ExistsUniqueTraced_07.json"
...
instance List.decidableBExistsUnique {α : Type*} [DecidableEq α] (p : α → Prop) [DecidablePred p] :
  ...
  | [] => .isFalse <| by simp_trace =>trace "test/SimpTrace/meas_out/ExistsUniqueTraced_08.json"
  | x :: xs =>
    if hx : p x then
