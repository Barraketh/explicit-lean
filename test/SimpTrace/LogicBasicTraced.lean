...
theorem and_symm_right {α : Sort*} (a b : α) (p : Prop) : p ∧ a = b ↔ p ∧ b = a := by simp_trace [eq_comm] =>trace "test/SimpTrace/meas_out/LogicBasicTraced_01.json"
theorem and_symm_left {α : Sort*} (a b : α) (p : Prop) : a = b ∧ p ↔ b = a ∧ p := by simp_trace [eq_comm] =>trace "test/SimpTrace/meas_out/LogicBasicTraced_02.json"
...
@[simp] theorem not_xor (P Q : Prop) : ¬Xor P Q ↔ (P ↔ Q) := by
  simp_trace only [not_and, Xor, not_or, not_not, ← iff_iff_implies_and_implies] =>trace "test/SimpTrace/meas_out/LogicBasicTraced_03.json"

theorem xor_iff_not_iff (P Q : Prop) : Xor P Q ↔ ¬(P ↔ Q) := (not_xor P Q).not_right

theorem xor_iff_iff_not : Xor a b ↔ (a ↔ ¬b) := by simp_trace only [← @xor_not_right a, not_not] =>trace "test/SimpTrace/meas_out/LogicBasicTraced_04.json"

theorem xor_iff_not_iff' : Xor a b ↔ (¬a ↔ b) := by simp_trace only [← @xor_not_left _ b, not_not] =>trace "test/SimpTrace/meas_out/LogicBasicTraced_05.json"
