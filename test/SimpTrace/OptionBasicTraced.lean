...
theorem mem_map {f : α → β} {y : β} {o : Option α} : y ∈ o.map f ↔ ∃ x ∈ o, f x = y := by simp_trace =>trace "test/SimpTrace/meas_out/OptionBasicTraced_01.json"
...
theorem forall_mem_map {f : α → β} {o : Option α} {p : β → Prop} :
    (∀ y ∈ o.map f, p y) ↔ ∀ x ∈ o, p (f x) := by simp_trace =>trace "test/SimpTrace/meas_out/OptionBasicTraced_02.json"

theorem exists_mem_map {f : α → β} {o : Option α} {p : β → Prop} :
    (∃ y ∈ o.map f, p y) ↔ ∃ x ∈ o, p (f x) := by simp_trace =>trace "test/SimpTrace/meas_out/OptionBasicTraced_03.json"
...
theorem map_injective' : Function.Injective (@Option.map α β) := fun f g h ↦
  funext fun x ↦ some_injective _ <| by simp_trace only [← map_some, h] =>trace "test/SimpTrace/meas_out/OptionBasicTraced_04.json"
...
theorem orElse_eq_some (o o' : Option α) (x : α) :
  ...
  simp_trace =>trace "test/SimpTrace/meas_out/OptionBasicTraced_05.json"

theorem orElse_eq_none (o o' : Option α) : (o <|> o') = none ↔ o = none ∧ o' = none := by
  simp_trace =>trace "test/SimpTrace/meas_out/OptionBasicTraced_06.json"
...
lemma getD_comp_some (d : α) : (fun x ↦ x.getD d) ∘ some = id := by
  ext
  simp_trace only [Function.comp_apply, getD_some, id_eq] =>trace "test/SimpTrace/meas_out/OptionBasicTraced_07.json"
