...
TIMEOUT = 900
EXPECTED_OVERRIDE_COUNT = 26
EXPECTED_MODULE_COUNT = 19
REQUIRED_OVERRIDE_OCCURRENCES = frozenset({
    "40d00689f4eea47b",
    "9872295221928046",
    "002698477f8acd5f",
    "acca7bbba4fc669d",
...
def assert_expected_cardinality(entries: list[dict[str, object]]) -> None:
    ...
    occurrences = {str(entry["occurrence"]) for entry in entries}
    missing = REQUIRED_OVERRIDE_OCCURRENCES - occurrences
    if missing:
        raise RuntimeError(
            "manual override checker is missing required entries: "
            + ", ".join(sorted(missing))
        )
...
def negative_controls(database: Path, work: Path) -> None:
    ...
    try:
        ...
        if "expected exactly 26 entries" not in str(error):
            raise
    else:
